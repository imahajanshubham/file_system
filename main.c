...
void fc_DisplayFileList( FCP_F *fcp_CurrF ) {
  ...
  for ( int counter = 1; counter <= fcp_CurrF -> f_Count; counter++  ) {
    ...
    printf( "| -> 🅵 %s\t\t%x\t%p\n",
            fcp_CurrF -> f_Child[ index ] -> f_Name,
            index,
...
void fc_UpdateFAT( FCP_F *fcp_CurrF,
                   int lv_NewFID ) {
...
void fc_PopulateFile( FCP_F *fcp_CurrF,
                      char *lv_NewFName ) {
  ...
  strcpy( fcp_CurrF -> f_Child[ lv_NewFID ] -> f_Name,
          lv_NewFName );
...
int fc_FileErrors( FCP_F *fcp_CurrF,
                   char *lv_NewFName ) {
...
int fc_CreateFile( FCP_F *fcp_CurrF,
                   char *lv_NewFName ) {
  ...
  fcp_CurrF -> f_Child[ lv_NewFID ] =
    ( FCP_F * )
    malloc( 250 * sizeof( FCP_F ) );
  ...
  fc_PopulateFile( fcp_CurrF,
                   lv_NewFName );

  fc_UpdateFAT( fcp_CurrF,
                lv_NewFID );
...
void fc_InitMF( FCP_F *fcp_MF,
                char *lv_NewFName ) {

  UI16 lv_NewFID = fc_HashCode( fcp_MF -> f_Name );


  strcpy( fcp_MF -> f_Name,
          lv_NewFName );
...
void fc_DisplayFCommands( void ) {
  ...
  printf( "\n%s%s%s%s%s%s",
          ...
          "❸ -> View directory contents with FIDs\n",
          "❹ -> Change directory\n",
          "❺ -> Switch current directory to MF\n",
...
int main( void ) {
  ...
  for( index = 0; index < 10; index++ )
    printf( "%X ", fc_InputCommand[ index ] );
