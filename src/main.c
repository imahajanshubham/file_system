...
struct directory {
    ...
    char
        name [ MAX_FILE_NAME ],
        *dir_list [ MAX_NODE_SIZE ];

    unsigned int
        id,
        dir_id [ MAX_NODE_SIZE ];
...
void update_fat ( struct directory *curr_dir,
                  char *dir_name,
                  int dir_id ) {
...
void populate_directory ( struct directory *curr_dir,
                          char *directory_name ) {

    int directory_id = hash_code( directory_name );

    strcpy( curr_dir -> inode[ directory_id ] . name,
            directory_name );
...
int directory_conflicts( struct directory *curr_dir,
                         char *directory_name ) {
    ...
    while ( curr_dir -> dir_list[ index ] != NULL ) {
        ...
        if ( ! strcmp( curr_dir -> dir_list[ index++ ] ,
                       directory_name ) )
            return EXISTS;
    }
...
void create_directory( struct directory *curr_dir,
                       char *directory_name ) {
    ...
    curr_dir -> inode = ( struct directory * )
        malloc( 250 * sizeof( struct directory ) );

    if ( directory_conflicts( curr_dir,
                              directory_name ) == EXISTS )
        return;

    populate_directory( curr_dir,
                        directory_name );
